...
description: "qrspi phase 7/8: execute the plan phase-by-phase with verification checkpoints, committing per phase. Reads plan.md. Run after qrspi-6-worktree. Next: qrspi-8-pr."
metadata:
  triggers:
    keywords:
    - qrspi 7 implement
    - workflow
---
# Qrspi 7 Implement Skill

> [!IMPORTANT]
> qrspi phase 7/8: execute the plan phase-by-phase with verification checkpoints, committing per phase. Reads plan.md. Run after qrspi-6-worktree. Next: qrspi-8-pr.
...
Read the `plan.md` file in the artifact directory. That is your primary working document.
...
- Tell the user: "Next: run the **qrspi-8-pr** workflow."
...
If a phase reveals the plan is fundamentally wrong — not a small mismatch but a structural issue like a missing dependency, wrong API, or incorrect assumption about the codebase — tell the user. For small mismatches, adapt and continue. For fundamental issues, suggest re-running the **qrspi-5-plan** workflow or even the **qrspi-3-design** workflow with the new information rather than building on a broken foundation.
